...
public class Task10GraphQLService {
    ...
    public Task10GraphQLService() {
        String sdl = """
            ...
            input CompetitorInput { id: ID!, name: String }
            type Event { id: ID!, source: String, status: Status, scheduled: String, competitors: [String], marketIds: [Int], descType: DescType! }
            type Mutation { editEvent(id: ID!, source: String, status: Status, scheduled: String, competitors: [CompetitorInput], marketIds: [Int], priority: Int, isLive: Boolean, notes: String, descType: DescType!): Event }
                        ...
                        .dataFetcher("editEvent", (DataFetcher<Map<String, Object>>) env -> {
                            ...
                            if (competitors != null) {
                                ...
                                for (Map<String, Object> c : competitors) {
                                    if (c.get("name") != null) names.add((String) c.get("name"));
                                }
                                result.put("competitors", names);
                            }
                            return result;
                        }))
                .type("Query", builder -> builder.dataFetcher("_empty", env -> ""))
                .build();
